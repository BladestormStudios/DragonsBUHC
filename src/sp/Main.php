...
{
    public $prefix = TE::GRAY . "[" . TE::AQUA . TE::BOLD . "" . TE::RED . "DragonsBUHC" . TE::RESET . TE::GRAY . "]";
    public $game = [];
    public $levels = array();
	...
	public function onEnable()
    {
        if ($this->getServer()->getPluginManager()->getPlugin("EconomyAPI") !== null) {
            $this->getLogger()->info("Plugin DragonsBUHC enabled!");
    ...
    public function onCommand(CommandSender $s, Command $cmd, string $label, array $args): bool
    {
        if ($cmd->getName() == "buhc") {
            ...
            if (count($args) < 2) {
                $s->sendMessage("/buhc make Arena Name");
                return false;
            }
            $level = $args[1];
    ...
    public function startGame(string $lvl): void
    {
        ...
        foreach ($this->getServer()->getLevelByName($lvl)->getPlayers() as $pl) {
			...
			$pl->getInventory()->addItem(Item::get(466, 0, 16));
			$pl->getInventory()->addItem(Item::get(176, 0, 2));
        }
    }
